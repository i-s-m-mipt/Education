...
//  ------------------------------------------------------------------------------------------
	auto & operator%=(Integer const & other)
	{
		...
		Integer product = quotient * other;
		*this -= product;
...
//  ------------------------------------------------------------------------------------------
	...
	friend Integer operator/ (Integer lhs, Integer const & rhs) 			{ return lhs /= rhs; }

	// Дружественный перегруженный оператор взятия остатка от деления (ТРЕБОВАНИЕ ЗАДАНИЯ)
	friend auto operator% (Integer lhs, Integer const & rhs) -> Integer 
	{ 
		return lhs %= rhs; 
	}
...
//  ------------------------------------------------------------------------------------------
	...
	// Функция для получения знака числа (ТРЕБОВАНИЕ ЗАДАНИЯ)
	int sign() const 
	{
		if (is_zero()) {
			return 0;
...
//  ------------------------------------------------------------------------------------------
	...
	// Функция для вычисления абсолютного значения (ТРЕБОВАНИЕ ЗАДАНИЯ)
	Integer abs() const 
	{
		Integer result = *this;
		if (result.m_is_negative) {
...
//  ------------------------------------------------------------------------------------------
	...
	// Функция для возведения в целую степень типа unsigned int (ТРЕБОВАНИЕ ЗАДАНИЯ)
...
//  ------------------------------------------------------------------------------------------
	...
	// Вспомогательная функция для проверки на ноль
	bool is_zero() const 
	{
...
//  ------------------------------------------------------------------------------------------
	...
	void reduce()
	{
		...
		// Если число равно 0, то оно не должно быть отрицательным
		if (is_zero()) {
			m_is_negative = false;
		}
	}
