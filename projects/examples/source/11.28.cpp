...
(
	auto && f, auto && pairs, std::vector < std::size_t > const & steps,

	std::index_sequence < Is ... >
)
{
	f(*std::next(std::get < Is > (pairs).first, steps[Is])...);
}
...
template < typename ... Is > auto cartesian(std::pair < Is, Is > ... pairs)
{
	...
	do
	{
		invoke(lambda, std::tie(pairs...), steps, std::make_index_sequence < sizeof...(pairs) > ());
	}
