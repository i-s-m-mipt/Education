...
private :
    ...
    template < typename T > static int handle_v1(T);

//  ----------------------------------------------------------------------------

	template < typename T1, typename T2 > static std::false_type handle_v2(...);

//  ----------------------------------------------------------------------------

    template < typename T1, typename T2 > static std:: true_type handle_v2
	(
		int, decltype(handle_v1 < T2 > (std::declval < T1 > ())) = 0
	);

public :
      
	constexpr static auto value = decltype(handle_v2 < D, B > (1))::value;
};
