//////////////////////////////////////////////////////////////////////

// chapter : Language Core Basics

//////////////////////////////////////////////////////////////////////

// content : Arithmetic Operators
//
// content : Arithmetic Operators with Assignment
//
// content : Postfix and Prefix Operators
//
// content : Comparison Operators
//
// content : Assembly Language Representation
//
// content : Service Compiler Explorer
//
// content : Options -S, -masm=intel and -masm=att

//////////////////////////////////////////////////////////////////////

#include <cassert>

//////////////////////////////////////////////////////////////////////

int main()
{
	auto x = 1, y = 2;

//  ------------------------------------------------------------------

	assert((x +  y) == +3); // support : https://compiler-explorer.com

	assert((x -  y) == -1); // support : https://compiler-explorer.com

	assert((x *  y) == +2); // support : https://compiler-explorer.com

	assert((x /  y) == +0); // support : https://compiler-explorer.com

	assert((x %  y) == +1); // support : https://compiler-explorer.com

//  ------------------------------------------------------------------

	assert((x += y) == +3);

	assert((x -= y) == +1);

	assert((x *= y) == +2);

	assert((x /= y) == +1);

	assert((x %= y) == +1);

//  ------------------------------------------------------------------

	assert((x ++  ) == +1);

	assert((x --  ) == +2);

	assert((  ++ y) == +3);

	assert((  -- y) == +2);

//  ------------------------------------------------------------------

	assert((x <  y) ==  1);

	assert((x >  y) ==  0);

	assert((x <= y) ==  1);

	assert((x >= y) ==  0);

	assert((x == y) ==  0);

	assert((x != y) ==  1);
}

//////////////////////////////////////////////////////////////////////