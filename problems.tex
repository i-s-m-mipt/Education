...
{\large \textbf{01.01} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/06.07.cpp}{06.07}\,)} \texttt{//} Stack Overflow} 
...
\newpage\thispagestyle{empty}\pdfpageheight = 26.90in\enlargethispage{100in}
...
дружественные для класса \lstinline{Entity} классы \lstinline{Tester_v1} и \lstinline{Tester_v2} для представления тестировщиков функций-членов \lstinline{test_v1} и \lstinline{test_v2} класса \lstinline{Entity} соответственно, чтобы получить прямой доступ к приватной секции класса \lstinline{Entity} и избежать использования публичного интерфейса класса \lstinline{Entity}. Используйте паттерн Attorney\,-\,Client для ограничения доступа классов \lstinline{Tester_v1} и \lstinline{Tester_v2} к приватной секции класса \lstinline{Entity}.

\bigskip

{\large \textbf{03.05} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/03.15.cpp}{03.15}\,)} \texttt{//} H. Sutter, Exceptional C\texttt{++}}
...
{\large \textbf{03.06} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/03.16.cpp}{03.16}\,)}}
...
{\large \textbf{03.08} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/03.28.cpp}{03.28}\,)}}

\bigskip

Реализуйте систему отложенного глубокого копирования строки при выполнении операции записи. Реализуйте класс \lstinline{String} для представления строки. Реализуйте структуру \lstinline{Data} для представления хранилища строки с подсчетом ссылок как приватную вложенную структуру в классе \lstinline{String}. Реализуйте в структуре \lstinline{Data} стандартную строку \lstinline{std::string} для хранения символов строки и поле стандартного типа \lstinline{std::size_t} для хранения значения счетчика ссылок с начальным значением единица. Реализуйте в классе \lstinline{String} приватный указатель типа \lstinline{Data *} для хранения адреса хранилища строки. Реализуйте в классе \lstinline{String} приватную функцию-член \lstinline{copy} для создания копии хранилища строки для текущего экземпляра класса \lstinline{String}, если значение счетчика ссылок оригинального хранилища строки больше единицы. Декрементируйте значение счетчика ссылок оригинального хранилища строки при создании копии. Реализуйте в классе \lstinline{String} пользовательский и копирующий конструкторы, деструктор и копирующий оператор присваивания. Инкрементируйте значение счетчика ссылок нового хранилища строки в копирующем конструкторе и копирующем операторе присваивания. Декрементируйте значение счетчика ссылок старого хранилища строки в деструкторе и копирующем операторе присваивания. Создавайте новое хранилище строки в пользовательском конструкторе. Уничтожайте старое хранилище строки при обнулении значения его счетчика ссылок. Используйте операторы \lstinline{new} и \lstinline{delete} для динамического выделения и освобождения памяти хранилищ строк. Реализуйте в классе \lstinline{String} публичные функции-члены \lstinline{get} и \lstinline{set} для выполнения операций чтения и записи соответственно. Используйте функцию-член \lstinline{copy} класса \lstinline{String} в функции-члене \lstinline{set} класса \lstinline{String} для глубокого копирования.
 
\bigskip

{\large \textbf{03.09} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/03.29.cpp}{03.29}\,)} \texttt{//} M. Bancila, The Modern C\texttt{++} Challenge}
...
стандартный контейнер \lstinline{std::array} с четырьмя элементами стандартного типа \lstinline{std::uint8_t} для хранения IP адресов. Реализуйте в классе \lstinline{IPv4} перегруженные операторы префиксного и постфиксного инкремента и декремента IP адресов. Реализуйте дружественные для класса \lstinline{IPv4} перегруженные операторы сравнения, ввода и вывода IP адресов. Используйте стандартный поток \lstinline{std::stringstream} для ввода через строку IP адресов в формате четырех целых чисел от 0 до 255 включительно, разделенных точками. Используйте стандартный поток \lstinline{std::stringstream} для вывода через строку IP адресов в формате, который использовался для ввода.

\bigskip

{\large \textbf{03.10} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/03.31.cpp}{03.31}\,)}}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/03.29.cpp}{\texttt{03.29}} таким образом, чтобы реализация использовала перегруженный оператор трехстороннего сравнения, переписывание выражений и перегруженный оператор сравнения на равенство вместо шести перегруженных операторов сравнения. Реализуйте дружественные для класса \lstinline{Rational} перегруженные 
...
{\large \textbf{04.02} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/04.05.cpp}{04.05}\,)}}
...
{\large \textbf{04.03} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/04.10.cpp}{04.10}\,)} \texttt{//} M. Bancila, The Modern C\texttt{++} Challenge}
...
{\large \textbf{04.04} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/04.11.cpp}{04.11}\,)} \texttt{//} M. Bancila, The Modern C\texttt{++} Challenge}
...
\newpage\thispagestyle{empty}\pdfpageheight = 9.00in\enlargethispage{100in}
...
Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/05.10.cpp}{\texttt{05.10}} таким образом, чтобы реализация использовала шаблон класса вместо ссылки. Реализуйте шаблон производного класса \lstinline{Decorator}, который является наследником интерфейса класса \lstinline{Entity} и наследником реализации собственного параметра шаблона. Используйте класс \lstinline{Entity} как виртуальный базовый класс. Удалите в классе \lstinline{Decorator} пользовательский конструктор и\:ссылку\:на\:экземпляр\:класса\:\lstinline{Entity}.
...
Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/05.15.cpp}{\texttt{05.15}} таким образом, чтобы реализация использовала статический полиморфизм вместо динамического полиморфизма. Удалите класс \lstinline{Strategy} и наследование от него всех остальных классов. Реа- 

лизуйте шаблон производного класса \lstinline{Entity}, который является наследником реализации собственного пара- метра\,шаблона.\,Удалите\,в\,классе\,\lstinline{Entity}\,пользовательский\,конструктор\,и\,ссылку\,на\,экземпляр\,класса\,\lstinline{Strategy}.
...
Доработайте Ваше предыдущее решение задачи 04.01 таким образом, чтобы пользователь мог выполнять тестирование алгоритма сортировки.\,Используйте\,библиотеку\,\href{https://github.com/google/googletest}{Google.Test}\,для\,реализации\,автоматических\,тестов.
...
филирование производительности алгоритма сортировки. Используйте библиотеку \href{https://github.com/google/benchmark}{Google.Benchmark} для реализации бенчмарков. Реализуйте параметризованный тест, передавая в качестве аргумента пороговый размер сортируемого сегмента контейнера при выборе между алгоритмом быстрой сортировки и алгоритмом сортировки вставками. Используйте контейнер из 10000 обратно сортированных элементов типа \lstinline{double} для тестов.



\newpage\thispagestyle{empty}\pdfpageheight = 11.80in\enlargethispage{100in}

\section{08. Applied Computations}

{\large \textbf{08.01} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/08.08.cpp}{08.08}\,)} \texttt{//} H. Sutter, Exceptional C\texttt{++} Style}
...
{\large \textbf{08.02} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/08.12.hpp}{08.12}\,)}}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/08.12.hpp}{\texttt{08.12}} таким образом, чтобы пользователь мог брать остаток от деления, возводить в степень и вычислять абсолютное значение длинных чисел. Реализуйте в классе \lstinline{Integer} перегруженный оператор взятия остатка от деления с присваиванием. Реализуйте дружественный для класса \lstinline{Integer} перегруженный оператор взятия остатка от деления. Реализуйте дружественную для класса \lstinline{Integer} функцию \lstinline{pow} для возведения длинного числа в целую степень типа \lstinline{unsigned int}. Реализуйте в классе \lstinline{Integer} публичные функции- члены \lstinline{sign} и \lstinline{abs} для получения знака и вычисления абсолютного значения длинного числа\:соответственно.

\bigskip

{\large \textbf{08.03} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/08.19.cpp}{08.19}\,)}}

\bigskip

Реализуйте алгоритмы вычисления целой части двоичного логарифма положительных чисел типа \lstinline{int} и типа \lstinline{float}. Предполагайте, что оба этих типа имеют размер 4 байта. Используйте значения типа \lstinline{unsigned int} для выполнения побитовых операций. Используйте оператор \lstinline{static_cast} для явного преобразования числа типа \lstinline{int} к значению типа \lstinline{unsigned int} и объединение \lstinline{union} для явного преобразования числа типа \lstinline{float} к значению типа \lstinline{unsigned int}. Используйте цикл \lstinline{while} и оператор побитового сдвига вправо для поиска старшего ненулевого бита в значении типа \lstinline{unsigned int}. Рассматривайте представление числа типа \lstinline{float} в соответствии со стандартом IEEE 754. Рассматривайте как нормализованные, так и денормализованные числа типа \lstinline{float}. Учитывайте, что экспонента числа типа \lstinline{float} имеет смещение на 127, которое обеспечивает хранение отрицательных степеней без знакового бита. Учитывайте, что максимальное значение экспоненты числа типа \lstinline{float} используется для представления значения бесконечности inf и неопределенного значения\,nan. Рассмотрите представление чисел с плавающей точкой в памяти компьютера, используя данную \href{https://cs357.cs.illinois.edu/textbook/assets/slides/94-Floating-Point.pdf}{презентацию}.

\bigskip

{\large \textbf{08.04} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/08.26.cpp}{08.26}\,)} \texttt{//} M. Bancila, The Modern C\texttt{++} Challenge}

\bigskip

Реализуйте алгоритм эволюции Ричарда Докинза, описанный им в третьей главе книги Слепой часовщик. Сгенерируйте начальную строку из 23 случайных букв. Сгенерируйте 100 копий начальной строки, заменяя каждую букву начальной строки другой случайной буквой с вероятностью 0.05. Вычислите для каждой из 100 сгенерированных строк значение метрики, показывающей посимвольное расхождение сгенерированной строки и целевой строкой methinksitislikeaweasel. Завершите работу алгоритма, если для любой из 100 сгенерированных строк значение метрики оказалось равным 0, в противном случае выберите любую сгенерированную строку с наименьшим значением метрики в качестве новой начальной строки и повторите итерацию алгоритма. Используйте только строчные буквы английского алфавита. Используйте стандартный источник энтропии \lstinline{std::random_device}. Используйте стандартный генератор \lstinline{std::default_random_engine}. Используйте стандартное распределение \lstinline{std::uniform_int_distribution} для реализации процесса генерации начальной строки и стандартное распределение \lstinline{std::uniform_real_distribution} для реализации процесса генерации новых строк. Используйте стандартный поток \lstinline{std::cout} для вывода через терминал строк на всех итерациях.

\bigskip

{\large \textbf{08.05} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/08.33.cpp}{08.33}\,)}}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/08.33.cpp}{\texttt{08.33}} таким образом, чтобы пользователь мог проводить серии измерений временных интервалов с последующим усреднением. Реализуйте в классе \lstinline{Timer} публичные функции-члены \lstinline{start} и \lstinline{stop} для запуска и остановки каждого измерения. Реализуйте в классе \lstinline{Timer} приватное поле типа \lstinline{bool} для проверки и обновления состояния таймера в функциях-членах \lstinline{start} и \lstinline{stop}. Реализуйте в классе \lstinline{Timer} приватный стандартный контейнер \lstinline{std::vector} для хранения стандартных интервалов \lstinline{std::chrono::duration} с типом \lstinline{double} в качестве первого параметра шаблона, соответствующих отдельным измерениям. Реализуйте в классе \lstinline{Timer} публичную функцию-член \lstinline{average} для вычисления и получения среднего значения\:времени\:в\:секундах.



\newpage\thispagestyle{empty}\pdfpageheight = 21.50in\enlargethispage{100in}
...
{\large \textbf{09.02} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.06.cpp}{09.06}\,)}}

\bigskip

Реализуйте класс \lstinline{Tree} для представления бинарного дерева. Реализуйте структуру \lstinline{Node} для представления узлов бинарного дерева как публичную вложенную структуру в классе \lstinline{Tree}. Реализуйте в структуре \lstinline{Node} поле типа \lstinline{int} для хранения значения текущего узла дерева, два стандартных указателя \lstinline{std::shared_ptr} для хранения адресов правого и левого дочерних узлов и стандартный указатель \lstinline{std::weak_ptr} для хранения адреса родительского узла. Реализуйте в классе \lstinline{Tree} публичный стандартный  указатель \lstinline{std::shared_ptr} для хранения адреса корневого узла дерева. Реализуйте в классе \lstinline{Tree} публичные функции-члены \lstinline{traverse_v1} и \lstinline{traverse_v2} для вывода значений всех текущих узлов дерева в окно терминала через стандартный поток \lstinline{std::cout} при полном обходе дерева от корня алгоритмами поиска в ширину и поиска в глубину соответственно. Сконструируйте в функции \lstinline{main} экземпляр класса \lstinline{Tree} таким образом, чтобы дерево содержало корневой узел, два дочерних узла на промежуточном уровне и четыре дочерних узла на последнем уровне. Продемонстрируйте отсутствие неразрывных связей между узлами дерева и корректную работу деструкторов.

\bigskip

{\large \textbf{09.03} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.10.cpp}{09.10}\,)}}

\bigskip

Доработайте примеры \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/05.01.cpp}{\texttt{05.01}}, \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/05.03.cpp}{\texttt{05.03}}, \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/05.04.cpp}{\texttt{05.04}}, \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/05.09.cpp}{\texttt{05.09}} и \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/05.13.cpp}{\texttt{05.13}} таким образом, чтобы реализации использовали стандартные интеллектуальные указатели вместо обычных указателей. Используйте стандартные указатели \lstinline{std::shared_ptr} и \lstinline{std::unique_ptr}. Не используйте стандартный указатель \lstinline{std::weak_ptr}. Подумайте над тем, какой стандартный интеллектуальный указатель будет уместно использовать в каждом из примеров.

\bigskip

{\large \textbf{09.04} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.12.cpp}{09.12}\,)}}

\bigskip

Доработайте Ваше предыдущее решение задачи 04.01 таким образом, чтобы реализация использовала итераторы вместо индексов. Реализуйте передачу коллекции элементов в алгоритм сортировки по значению двух итераторов начала и конца коллекции. Используйте полуоткрытые диапазоны. Предполагайте, что элементы коллекции могут храниться в любом контейнере, который обладает итераторами произвольного доступа. Используйте стандартные функции \lstinline{std::distance}, \lstinline{std::advance}, \lstinline{std::next}, \lstinline{std::prev} и \lstinline{std::iter_swap}.

\bigskip

{\large \textbf{09.05} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.16.cpp}{09.16}\,)}}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.16.cpp}{\texttt{09.16}} таким образом, чтобы реализация использовала двунаправленные итераторы вместо однонаправленных итераторов. Реализуйте класс \lstinline{List} для представления двусвязного списка. Реализуйте в структуре \lstinline{Node} стандартный указатель \lstinline{std::weak_ptr} для хранения адреса предыдущего узла списка. Реализуйте в классе \lstinline{Iterator} перегруженные операторы префиксного и постфиксного декремента.~Замените~в классе \lstinline{List} псевдоним категории итератора. Доработайте в классе \lstinline{List} реализацию~функции-члена~\lstinline{push_back}.

\bigskip

{\large \textbf{09.06} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.17.cpp}{09.17}\,)}}

\bigskip

Реализуйте алгоритм вычисления чисел ряда Фибоначчи на основе однонаправленного итератора. Реализуйте класс \lstinline{Iterator} для представления однонаправленного итератора. Реализуйте в классе \lstinline{Iterator} два приватных поля типа \lstinline{int} для хранения двух смежных чисел ряда Фибоначчи. Реализуйте в классе \lstinline{Iterator} конструктор по умолчанию, перегруженные операторы префиксного и постфиксного инкремента, разыменования и сравнения на равенство. Реализуйте операторы инкремента таким образом, чтобы они вычисляли следующие два смежных числа ряда Фибоначчи на основе двух текущих смежных чисел ряда Фибоначчи, а оператор разыменования таким образом, чтобы он возвращал последнее вычисленное число ряда Фибоначчи. Реализуйте альтернативный алгоритм вычисления чисел ряда Фибоначчи на основе однонаправленного итератора. Используйте библиотеку \href{https://www.boost.org/doc/libs/latest/libs/iterator/doc/index.html}{Boost.Iterator} для реализации интерфейса класса \lstinline{Iterator} на основе фасада.

\bigskip

{\large \textbf{09.07} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.22.cpp}{09.22}\,)} \texttt{//} H. Sutter, Exceptional C\texttt{++}}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/06.09.hpp}{\texttt{06.09}} таким образом, чтобы реализация использовала повторно  однократно выделенную неинициализированную память для хранения деталей реализации основного класса вместо многократного выделения и освобождения памяти. Обоснуйте накладные расходы и влияние на производительность оригинального паттерна Pimpl. Реализуйте в классе \lstinline{Entity} приватный стандартный контейнер \lstinline{std::array} со спецификатором \lstinline{alignas(std::max_align_t)} и шестнадцатью элементами стандартного типа \lstinline{std::byte} для хранения экземпляра класса \lstinline{Implementation}. Реализуйте в конструкторе класса \lstinline{Entity} два статических утверждения \lstinline{static_assert} для проверки размера и выравнивания класса \lstinline{Implementation}. Используйте размещающую версию оператора \lstinline{new} в конструкторе класса \lstinline{Entity}. Используйте стандартную функцию \lstinline{std::destroy_at} в деструкторе класса \lstinline{Entity}. Реализуйте в классе \lstinline{Entity} константную и неконстантную публичные функции-члены \lstinline{get} для получения отмытого указателя на экземпляр класса~\lstinline{Implementation}.~Ис-

пользуйте стандартные функции \lstinline{std::bit_cast} и \lstinline{std::launder} в обеих функциях-членах \lstinline{get} класса \lstinline{Entity}.

\bigskip

{\large \textbf{09.08} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.24.cpp}{09.24}\,)}}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.24.cpp}{\texttt{09.24}} таким образом, чтобы пользователь мог обслуживать память встроенных динамических массивов с помощью перегруженных функций выделения и освобождения памяти. Реализуйте в классе \lstinline{Entity} публичные статические функции-члены \lstinline{operator new[]} и \lstinline{operator delete[]} для выделения и освобождения памяти встроенных динамических массивов. Реализуйте в классе \lstinline{Entity} публичные статические функции-члены \lstinline{operator new}, \lstinline{operator delete}, \lstinline{operator new[]} и \lstinline{operator delete[]} для выделения и освобождения памяти встроенных динамических массивов без генерации исключений, которые имеют дополнительный второй параметр стандартного типа \lstinline{std::nothrow_t}, передаваемый с помощью константной lvalue\,-\,ссылки. Реализуйте в классе \lstinline{Client} все необходимые дополнительные публичные объявления\:\lstinline{using}.

\bigskip

{\large \textbf{09.09} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.32.cpp}{09.32}\,)}}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.32.cpp}{\texttt{09.32}} таким образом, чтобы пользователь мог выбирать алгоритм поиска свободных блоков памяти. Реализуйте в классе \lstinline{Allocator} приватную функцию-член \lstinline{find_first} для представления алгоритма поиска первого подходящего свободного блока памяти. Реализуйте в классе \lstinline{Allocator} приватную функцию-член \lstinline{find_best} для представления алгоритма поиска лучшего подходящего свободного блока памяти. Реализуйте в конструкторе класса \lstinline{Allocator} дополнительный параметр для выбора алгоритм поиска свободных блоков памяти. Сравните среднее время работы аллокатора, использующего первый алгоритм поиска свободных блоков памяти, и среднее время работы того же аллокатора, использующего второй алгоритм поиска, в рамках одинаковых тестов. Используйте библиотеку \href{https://github.com/google/benchmark}{Google.Benchmark} для реализации бенчмарков.

\bigskip

{\large \textbf{09.10} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.34.cpp}{09.34}\,)}}

\bigskip

Доработайте примеры \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.29.cpp}{\texttt{09.29}}, \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.30.cpp}{\texttt{09.30}}, \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.31.cpp}{\texttt{09.31}} и \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/09.32.cpp}{\texttt{09.32}} таким образом, чтобы реализации использовали наследование интерфейсов от общего абстрактного базового класса вместо дублирования интерфейсов в отдельных самостоятельных классах. Реализуйте абстрактный базовый класс \lstinline{Allocator} для представления интерфейса аллокаторов. Реализуйте в классе \lstinline{Allocator} виртуальный деструктор и публичные чисто виртуальные функции-члены \lstinline{allocate} и \lstinline{deallocate} для выделения и освобождения памяти соответственно. Реализуйте в классе \lstinline{Allocator} защищенный шаблон функции-члена \lstinline{get} для получения указателя на объект стандартного типа \lstinline{std::byte} или пользовательских типов \lstinline{Header} или \lstinline{Node}. Реализуйте четыре производных класса аллокаторов для представления различных методов выделения и освобождения памяти, которые являются наследниками интерфейса класса \lstinline{Allocator}. Продемонстрируйте использование полиморфных аллокаторов.

\newpage\thispagestyle{empty}\pdfpageheight = 7.40in\enlargethispage{100in}

\section{10. Programming with Containers}

{\large \textbf{10.01} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/10.07.cpp}{10.07}\,)}}

\bigskip

Исследуйте систему автоматического увеличения емкости памяти стандартного контейнера \lstinline{std::vector}. Определите коэффициент умножения емкости памяти контейнера в случае нехватки свободных ячеек памяти. Реализуйте тесты для отслеживания изменений емкости памяти контейнера в процессе вставки новых элементов. Исследуйте систему автоматического увеличения емкости памяти стандартного контейнера \lstinline{std::deque}. Определите размер страниц непрерывной памяти, используемых для хранения групп смежных элементов контейнера. Реализуйте тесты для отслеживания адресов элементов контейнера в процессе вставки новых элементов.

\bigskip

{\large \textbf{10.02} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/10.28.cpp}{10.28}\,)} \texttt{//} M. Bancila, The Modern C\texttt{++} Challenge}

\bigskip

Реализуйте алгоритм моделирования игры Жизнь по стандартным правилам на игровом поле размером 10 на 10 клеток. Используйте библиотеку \href{https://www.boost.org/doc/libs/latest/libs/multi_array/doc/user.html}{Boost.MultiArray} для реализации двумерного хранилища состояний игрового поля. Установите вручную некоторый паттерн начального состояния~игрового~поля.~Используйте~стандарт- 

ный поток \lstinline{std::cout} для вывода через терминал состояний игрового поля на всех итерациях моделирования.

\bigskip

{\large \textbf{10.03} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/10.31.cpp}{10.31}\,)}}

\bigskip

Реализуйте алгоритм вычисления N\,-\,ого числа ряда Фибоначчи на основе метода матричной экспоненциации. Используйте библиотеку \href{https://www.boost.org/doc/libs/latest/libs/numeric/ublas/doc/}{Boost.UBLAS} для реализации матричных вычислений. Используйте тип \lstinline{usigned long long int} для значений элементов матриц. Реализуйте алгоритм быстрого возведения начальной матрицы в степень N. Обоснуйте алгоритмическую сложность реализованного алгоритма и сравните ее с алгоритмической сложностью всех остальных известных Вам алгоритмов вычисления N\,-\,ого числа ряда Фибоначчи.

\bigskip

{\large \textbf{10.04} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/10.39.cpp}{10.39}\,)}}
...
{\large \textbf{10.05} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/10.42.cpp}{10.42}\,)}}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/10.42.cpp}{\texttt{10.42}} таким образом, чтобы пользователь мог исследовать девять хэш-функций, приведенных в данной \href{https://www.partow.net/programming/hashfunctions/#AvailableHashFunctions}{статье}. Постройте графики зависимости количества возникающих коллизий от количества хэшируемых строк. Обоснуйте форму полученных зависимостей. Определите лучшие и худшие хэш-функции данного набора. Выполните сборку решения с флагами -O3 и -m32 из-за особенностей некоторых хэш-функций.



\newpage\thispagestyle{empty}\pdfpageheight = 10.20in\enlargethispage{100in}
...
Реализуйте функцию, которая возвращает указатель на себя таким образом, чтобы следующий код работал: \lstinline{Wrapper function = test(); (*function)();} Реализуйте класс \lstinline{Wrapper} для представления оболочки вок- 

руг указателя на функцию \lstinline{test}. Реализуйте в классе \lstinline{Wrapper} перегруженный оператор преобразования типа.

\bigskip

{\large \textbf{11.02} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/11.10.cpp}{11.10}\,)}}

\bigskip

Доработайте Ваше предыдущее решение задачи 09.04 таким образом, чтобы пользователь мог передавать собственный компаратор в алгоритм сортировки для сравнения элементов. Используйте шаблоны функций. Продемонстрируйте передачу в алгоритм сортировки пользовательской свободной функцией, стандартного функционального объекта \lstinline{std::less} и пользовательской лямбда-функции в роли собственных компараторов.

\bigskip

{\large \textbf{11.03} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/11.13.cpp}{11.13}\,)}}

\bigskip

Доработайте Ваше предыдущее решение задачи 07.01 таким образом, чтобы реализация использовала паттерн Visitor для извлечения корней из варианта вместо ручной распаковки варианта. Реализуйте класс \lstinline{Visitor} для представления посетителя. Реализуйте в классе \lstinline{Visitor} три перегруженных оператора вызова для обработки каждого из возможных значений варианта. Используйте стандартную функцию \lstinline{std::visit} для применения перегруженных операторов вызова экземпляра класса \lstinline{Visitor} к варианту, который был получен из опционала.

\bigskip

{\large \textbf{11.04} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/11.26.cpp}{11.26}\,)}}

\bigskip

Продемонстрируйте использование диапазонов и стандартных алгоритмов \lstinline{ranges::replace}, \lstinline{ranges::fill}, \lstinline{ranges::unique}, \lstinline{ranges::rotate} и \lstinline{ranges::sample} на простых тестах. Реализуйте алгоритм \lstinline{transform_if} на основе комбинации стандартных алгоритмов \lstinline{ranges::transform} и \lstinline{ranges::copy_if}. Реализуйте алгоритмы вычисления средней абсолютной ошибки MAE и среднеквадратичной ошибки MSE на основе комбинации стандартных численных алгоритмов. Продемонстрируйте использование диапазонов и стандартных представлений \lstinline{views::filter}, \lstinline{views::drop}, \lstinline{views::join}, \lstinline{views::zip} и \lstinline{views::stride} на простых тестах. Доработате Ваше предыдущее решение задачи 09.06 таким образом, чтобы пользователь мог вычислять числа ряда Фибоначчи на основе представления. Используйте стандартный базовый класс \lstinline{std::ranges::view_interface} для определения интерфейса представления через странно рекурсивный шаблон проектирования. Реализуйте производный класс \lstinline{Fibonacci} для представления алгоритма вычисления чисел ряда Фибоначчи. Реализуйте класс \lstinline{Iterator} для представления однонаправленного итератора как приватный вложенный класс в классе \lstinline{Fibonacci}. Реализуйте в классе \lstinline{Fibonacci} публичные функции-члены \lstinline{begin} и \lstinline{end} для создания итераторов.

\bigskip

{\large \textbf{11.05} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/11.29.cpp}{11.29}\,)}}

\bigskip

Реализуйте алгоритм решения задачи коммивояжера для неориентированного полносвязного графа, содержащего 10 вершин. Используйте библиотеку \href{https://www.boost.org/doc/libs/latest/libs/graph/doc/index.html}{Boost.Graph} для реализации графа. Используйте матрицу смежности для хранения графа. Используйте тип \lstinline{int} для значений весов ребер графа. Инициализируйте веса ребер графа случайными значениями от 1 до 10. Используйте стандартный источник энтропии \lstinline{std::random_device}. Используйте стандартный генератор \lstinline{std::default_random_engine}. Используйте стандартное распределение \lstinline{std::uniform_int_distribution} для реализации процесса генерации весов ребер графа. Используйте стандартный алгоритм \lstinline{std::next_permutation} для перебора всех перестановок последовательности обхода вершин, включающей в себя каждую вершину графа только один раз. Учтите, что обход вершин графа должен завершаться в начальной вершине. Используйте стандартный поток \lstinline{std::cout} для вывода через терминал матрицы инцидентности, оптимальной последовательности обхода вершин, а также ее суммарной стоимости.
...
{\large \textbf{12.01} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/12.01.cpp}{12.01}\,)}}

\bigskip

Реализуйте алгоритм конвертации валюты RUB в валюту USD с поддержкой ввода и вывода локализованных обозначений денежных сумм. Используйте стандартные локали \lstinline{std::locale} для настройки потоков ввода и вывода. Используйте русскую локаль \lstinline{ru_RU.utf8} для настройки потока ввода. Используйте американскую локаль \lstinline{en_US.utf8} для настройки потока вывода. Используйте стандартные манипуляторы \lstinline{std::get_money} и \lstinline{std::put_money} для ввода и вывода. Используйте стандартные потоки \lstinline{std::stringstream} для реализации потоков ввода и вывода. Указывайте обозначение валюты RUB до или после указания денежной суммы в RUB.

\bigskip

{\large \textbf{12.02} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/12.09.cpp}{12.09}\,)} \texttt{//} Stack Overflow}

\bigskip

Напишите программу, которая выводит собственный исходный код в окно терминала через стандартный поток \lstinline{stdout}. Используйте стандартную функцию \lstinline{std::printf}. Не используйте файловые потоки ввода и вывода.

\bigskip

{\large \textbf{12.03} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/12.11.cpp}{12.11}\,)} \texttt{//} M. Bancila, The Modern C\texttt{++} Challenge}

\bigskip

Реализуйте алгоритм поиска подстроки-палиндрома наибольшей длины. Не используйте полный перебор. Используйте кэширование для уменьшения алгоритмической сложности. Используйте стандартный контейнер \lstinline{std::bitset} для представления линеаризованной таблицы кэширования размера N на N, где N - количество символов в строке. Используйте стандартное представление \lstinline{std::string_view}\,для\,оптимизации\,копирования.

\bigskip

{\large \textbf{12.04} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/12.18.cpp}{12.18}\,)}}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/12.18.cpp}{\texttt{12.18}} таким образом, чтобы пользователь мог извлечь все адреса электронных почт и их домены из некоторого текста. Используйте упрощенный формат адресов электронных почт. Используйте группу для извлечения доменов. Используйте сырые строковые литералы для реализации текстов для тестов.

\bigskip

{\large \textbf{12.05} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/12.26.cpp}{12.26}\,)}}

\bigskip

Доработайте примеры \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/12.19.cpp}{\texttt{12.19}} и \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/12.26.cpp}{12.26} таким образом, чтобы пользователь мог брать остаток от деления, возводить в степень и вычислять факториал чисел, а также использовать квадратные и фигурные скобки. Реализуйте грамматический разбор и вычисление выражений на основе бинарного оператора процент для взятия остатка от деления первого операнда на второй, бинарного оператора исключающее или для возведения первого операнда в степень второго и унарного постфиксного оператора восклицательный знак для вычисления факториала операнда. Не рассматривайте проблемы переполнения и точности типа \lstinline{double} при вычислении факториала чисел. Реализуйте грамматический разбор и вычисление выражений на основе квадратных и фигурных скобок. Используйте \href{https://github.com/Chrinkus/stroustrup-ppp/blob/master/chapter07/ch07_calculator.cpp}{реализацию} рекурсивного калькулятора Бьерна Страуструпа в качестве подсказки.



\newpage\thispagestyle{empty}\pdfpageheight = 8.60in\enlargethispage{100in}

\section{13. Input and Output Subsystems}

{\large \textbf{13.01} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/13.03.cpp}{13.03}\,)} \texttt{//} M. Bancila, The Modern C\texttt{++} Challenge}

\bigskip

Реализуйте алгоритм преобразования коллекции восьмибитных беззнаковых целых чисел в строку с их шестнадцатиричными представлениями. Используйте стандартный контейнер \lstinline{std::vector} с элементами стандартного типа \lstinline{std::uint8_t} для хранения коллекции восьмибитных беззнаковых целых чисел. Используйте стандартный поток \lstinline{std::stringstream} для создания необходимой строки. Используйте стандартные манипуляторы \lstinline{std::setw}, \lstinline{right}, \lstinline{std::setfill} и \lstinline{std::hex} для форматирования вывода. Реализуйте обратный алгоритм преобразования строки, состоящей из четного количества шестнадцатиричных цифр, в коллекцию восьмибитных беззнаковых целых чисел. Используйте вычитание символов и побитовые операции для реализа- ции обратного алгоритма преобразования. Используйте только шестнадцатиричные цифры в нижнем регистре.  

\bigskip

{\large \textbf{13.02} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/13.09.cpp}{13.09}\,)}}

\bigskip

Доработайте Ваше предыдущее решение задачи 12.05 таким образом, чтобы реализация использовала файловый поток ввода для чтения инструкций и их результатов из файла вместо взаимодействия с пользователем через терминал. Используйте стандартный поток \lstinline{std::fstream} в режиме ввода для чтения тестовых~данных.

\bigskip

{\large \textbf{13.03} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/13.15.cpp}{13.15}\,)}}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/13.15.cpp}{\texttt{13.15}} таким образом, чтобы реализация использовала алгоритм удаления пустых строк и строк, состоящих из пробельных символов, вместо сохранения их в итоговом файле. Реализуйте обработку сырых строковых литералов в алгоритме. Рассмотрите особенности оформления сырых строковых литералов.

\bigskip

{\large \textbf{13.04} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/13.18.cpp}{13.18}\,)} \texttt{//} J. Galowicz, C\texttt{++}17 STL Cookbook}

\bigskip

Доработайте пример \href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/13.18.cpp}{\texttt{13.18}} таким образом, чтобы реализация использовала алгоритм условного отображения только тех вхождений в директорию, названия которых соответствуют заданному пользователем регулярному выражению, вместо безусловного отображения всех вхождений. Используйте стандартное регулярное выражение \lstinline{std::regex} и стандартную функцию \lstinline{std::regex_match} для реализации алгоритма поиска соответствий.

\bigskip

{\large \textbf{13.05} \texttt{(\,\href{https://github.com/i-s-m-mipt/Education/blob/master/projects/examples/source/13.20.cpp}{13.20}\,)}}

\bigskip

Реализуйте алгоритм чтения целого числа и числа с плавающей точкой из строки, сериализованной в формате JSON. Предполагайте, что расположение чисел внутри строки известно. Реализуйте обратный алгоритм записи целого числа и числа с плавающей точкой на известные позиции в строку, сериализованную в формате JSON. Предполагайте, что все числа положительные, а количество знаков до и после точки числа с плавающей точкой известно. Используйте тип \lstinline{int} для представления целого числа и тип \lstinline{float} для представления числа с плавающей точкой. Используйте алгоритмы библиотеки \href{https://github.com/nlohmann/json}{Nlohmann.JSON}, алгоритмы других библиотек, стандартные функции \lstinline{std::from_chars} и \lstinline{std::to_chars}, а также собственные алгоритмы. Используйте~биб- 

лиотеку \href{https://github.com/google/benchmark}{Google.Benchmark} для реализации бенчмарков. Постарайтесь реализовать все алгоритмы оптимально.
