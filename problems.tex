...
\begin{itemize}
    ...
    \item Ваши решения не должны содержать стандартные символьные потоки ввода и вывода.
...
\newpage\thispagestyle{empty}\pdfpageheight = 7.00in\enlargethispage{100in}
...
Реализуйте структуру \lstinline{Rectangle} для представления прямоугольников со сторонами, которые параллельны осям координатной плоскости. Реализуйте поля типа \lstinline{double} для хранения координат левого верхнего и правого нижнего углов прямоугольника. Реализуйте алгоритм вычисления площади пересечения нескольких прямоугольников. Используйте контейнер \lstinline{std::vector} для хранения и обработки экземпляров структуры \lstinline{Rectangle}. Реализуйте корректное сравнение чисел типа \lstinline{double}. Рассмотрите случаи пустых и  вырожденных пересечений.
...
Реализуйте классы \lstinline{Triangle}, \lstinline{Square} и \lstinline{Circle} для представления треугольников, квадратов и окружностей. Реализуйте поля типа \lstinline{double} для хранения длин трех сторон в классе \lstinline{Triangle}, длины одной стороны в классе \lstinline{Square} и радиуса в классе \lstinline{Circle}. Реализуйте алгоритмы вычисления периметра и площади всех указанных фигур как функции-члены соответствующих классов. Используйте стандартную константу \lstinline{std::numbers::pi}.
...
Реализуйте структуру \lstinline{Node} для представления узлов односвязного списка. Реализуйте поле типа \lstinline{int} для хранения значения текущего узла списка и поле типа \lstinline{Node *} для хранения адреса следующего узла списка. Реализуйте класс \lstinline{List} для представления односвязного списка. Реализуйте поле типа \lstinline{Node *} для хранения адреса головного узла списка. Не создавайте поле для хранения текущей длины списка. Реализуйте функцию-член \lstinline{push} для добавления новых узлов в конец списка. Используйте оператор \lstinline{new} для динамического выделения памяти в функции \lstinline{push}. Реализуйте алгоритм вычисления указателя на средний узел списка как функцию-член класса \lstinline{List}. Используйте только один цикл в алгоритме для обхода списка. Реализуйте деструктор, который освободит память, выделенную при создании узлов в функции \lstinline{push}. Используйте рекурсивный обход списка.
